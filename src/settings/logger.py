...
file_formatter = logging.Formatter('%(levelname)s - %(filename)s - %(message)s')
...
class DokoolaLogger:
    ...
    def __log(cls, log_attr, message, extras):
        try:
            ...
            if extras:
                logging.getLogger("logtail").__getattribute__(log_attr)(message, extra=extras)
        except:

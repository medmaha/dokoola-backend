...
class ThreadListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Thread.objects.filter(owner=user, messenging__gt=0).distinct()
...
class ThreadMessagesAPIView(ListAPIView):
    ...
    def get_queryset(self, query_params):
        ...
        if thread:
            queryset = thread.messenging.all().order_by("-created_at")
            return queryset
        return
...
class MessengingCreateAPIView(CreateAPIView):
    serializer_class = MessengingCreateSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            message = Messenging.objects.create(**message_data)

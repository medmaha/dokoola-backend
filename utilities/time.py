...
def utc_datetime(
    timestamp: Optional[float] = None, add_minutes: Optional[int] = None
) -> datetime:
    if timestamp:
...
__all__ = ["utc_datetime", "utc_timestamp"]

DEFAULT_ADD_MINUTE = 5


def add_minutes_to_timestamp(
    timestamp: float, minutes: Optional[float] = None
) -> float:
    if not minutes:
